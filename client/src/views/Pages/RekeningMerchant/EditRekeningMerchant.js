...
const EditRekeningMerchant = (props) => {
  const timeout = 300;
  const fadeIn = true;
  const collapse = true;
  ...
  const submitData = async (e) => {
    ...
    e.preventDefault();
    try {
      const result = await axios.put(`${API_SERVER}/rekening-merchant/${id}`,{
          name: ownerName,
          number: number,
          bank_id: bankId,
          active: status
  ...
  const deleteDataUser = async () => {
    const id = props.match.params.id;
    try {
      const result = await axios.delete(`${API_SERVER}/rekening-merchant/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      })
                ...
                <CardBody>
                  <Form onSubmit={submitData} className="form-horizontal">
                    ...
                    <FormGroup>
                      ...
                      <div className="controls">
                        <InputGroup>
                          <Input value={bankName} name="phonenumber" id="bankName" type="text" disabled required />
                        </InputGroup>
                      </div>
                    </FormGroup>
                    <FormGroup>
...
export default EditRekeningMerchant;
